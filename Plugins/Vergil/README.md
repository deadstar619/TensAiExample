...
- Run the persisted compile/apply recovery regression test: `powershell -ExecutionPolicy Bypass -File .\Plugins\Vergil\Tools\Invoke-VergilScaffoldAutomation.ps1 -TestFilter 'CompileApplyRecoveryRoundtrip'`
...
- Large-graph benchmarks only: `powershell -ExecutionPolicy Bypass -File .\Plugins\Vergil\Tools\Invoke-VergilCILane.ps1 -Lane Benchmarks`

The `Benchmarks` lane runs `Vergil.Scaffold.LargeGraphBenchmark`, which measures large synthetic graphs under the full compile-planning plus direct-apply surfaces and writes a structured summary to `Saved/Logs/VergilCI/Benchmarks/LargeGraphBenchmarkSummary.json`.
...
- See [EXTENDING_NODE_HANDLERS.md](EXTENDING_NODE_HANDLERS.md) for the supported workflow when adding a new `IVergilNodeHandler`, including when to use the shared generic node spawner, a specialized handler, or direct lowering.
...
- The current whole-asset authoring surface now has persisted save/reload/native-compile roundtrip coverage on a real `/Game/Tests/...` Blueprint package plus save/reload recovery coverage for failed compile/apply attempts and checked-in golden snapshot and reviewed source-control diff fixtures under `Plugins/Vergil/Tests/GoldenAssets/`.
...
- `VGR-1001`, `VGR-1002`, `VGR-1003`, `VGR-1004`, `VGR-1005`, `VGR-1006`, `VGR-1007`, `VGR-1008`, `VGR-1009`, `VGR-1010`, `VGR-2001`, `VGR-2002`, `VGR-2003`, `VGR-2004`, `VGR-3001`, `VGR-3002`, `VGR-3003`, `VGR-3004`, `VGR-3005`, `VGR-3006`, `VGR-3007`, `VGR-3008`, `VGR-4001`, `VGR-4002`, `VGR-4003`, `VGR-4004`, `VGR-4005`, `VGR-4006`, `VGR-4007`, `VGR-4008`, `VGR-4009`, `VGR-5001`, `VGR-5002`, `VGR-5003`, `VGR-5004`, `VGR-5005`, `VGR-5006`, `VGR-5007`, `VGR-5008`, `VGR-6001`, `VGR-6002`, `VGR-6003`, `VGR-6004`, `VGR-6005`, `VGR-6006`, `VGR-6007`, `VGR-6008`, `VGR-7001`, `VGR-7002`, `VGR-7003`, `VGR-7004`, `VGR-7005`, `VGR-7006`, `VGR-7007`, `VGR-8001`, `VGR-8002`, `VGR-8003`, `VGR-8004`, `VGR-8005`, `VGR-8006`, `VGR-8007`, `VGR-8008`, `VGR-9001`, `VGR-9002`, `VGR-9003`, `VGR-9004`, `VGR-9005`, `VGR-9006`, `VGR-9007`, `VGR-9008`, `VGR-9009`, and `VGR-9010` are complete.
...
- Vergil now also has an explicit handler-extension guide that documents the supported `IVergilNodeHandler` and `FVergilNodeRegistry` seam, how new families should choose between generic spawner, specialized-handler, direct-lowering, or unsupported matrix rows, and which compiler, executor, manifest, doc, and automation updates are required before a handler is considered done.
...
- The scaffold now also has dedicated large-graph benchmark coverage: `Vergil.Scaffold.LargeGraphBenchmark` measures two synthetic comment/layout-enabled event-graph scenarios, records plan and apply timings to `Saved/Vergil/Benchmarks/LargeGraphBenchmarkSummary.json`, and the checked-in `Benchmarks` CI lane copies that summary into `Saved/Logs/VergilCI/Benchmarks/`.
- Repo-level GitHub Actions CI now exists for a self-hosted Windows Unreal runner, with explicit `Build`, `HeadlessAutomation`, `GoldenTests`, `PIRuntime`, and `Benchmarks` lanes that reuse checked-in PowerShell entrypoints instead of duplicating commands inside workflow YAML.
