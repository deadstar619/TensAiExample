...
namespace
{
	...
	const FName AIMoveToDescriptor(TEXT("K2.AIMoveTo"));
	const FName PlayMontageDescriptor(TEXT("K2.PlayMontage"));
	const FName AsyncActionCommandName(TEXT("Vergil.K2.AsyncAction"));
	const FName AIMoveToCommandName(TEXT("Vergil.K2.AIMoveTo"));
	...
	struct FVergilComponentLookupDescriptor
	{
		FName Descriptor;
		FName CommandName;
		FName FunctionName;
	...
	struct FVergilSpecializedAsyncTaskDescriptor
	{
		FName Descriptor;
		FName CommandName;
	};
	...
	const FVergilSpecializedAsyncTaskDescriptor* FindSpecializedAsyncTaskDescriptorByDescriptor(const FName Descriptor)
	{
		static const FVergilSpecializedAsyncTaskDescriptor Descriptors[] =
		{
			{ AIMoveToDescriptor, AIMoveToCommandName },
			{ PlayMontageDescriptor, PlayMontageCommandName },
		};

		for (const FVergilSpecializedAsyncTaskDescriptor& Candidate : Descriptors)
		{
			if (Candidate.Descriptor == Descriptor)
			{
				return &Candidate;
		...
		const bool bIsExec)
	{
		...
		if (!OutPins.ContainsByPredicate([Name, Direction, bIsExec](const FVergilSupportedPinDescriptor& Candidate)
		{
			return Candidate.Name == Name && Candidate.Direction == Direction && Candidate.bIsExec == bIsExec;
		}))
		{
			OutPins.Add({ Name, Direction, bIsExec });
		}
	}
	...
	void GatherHiddenAsyncTaskPins(const UFunction* FactoryFunction, TSet<FName>& OutHiddenPins)
	{
	...
	bool DoesAsyncTaskHideThen(const UClass* ProxyClass)
	{
	...
	bool DoesAsyncTaskExposeProxy(const UClass* ProxyClass)
	{
		for (const UStruct* TestStruct = ProxyClass; TestStruct != nullptr; TestStruct = TestStruct->GetSuperStruct())
		{
		...
		UClass*& OutProxyClass)
	{
		...
		if (FactoryClass == nullptr)
		{
			Context.AddDiagnostic(
				...
				TEXT("AsyncActionFactoryClassMissing"),
				TEXT("Async action nodes require a resolved FactoryClassPath."),
				Node.Id);
			return false;
		}

		if (FactoryFunctionName.IsNone())
		{
			Context.AddDiagnostic(
				EVergilDiagnosticSeverity::Error,
				TEXT("MissingAsyncActionFactoryFunction"),
				TEXT("Async action nodes require a descriptor suffix naming the factory function."),
				Node.Id);
			return false;
		}
		...
		UFunction* const FactoryFunction = FactoryClass->FindFunctionByName(FactoryFunctionName);
		if (FactoryFunction == nullptr)
		{
			Context.AddDiagnostic(
				EVergilDiagnosticSeverity::Error,
				...
				FString::Printf(
					TEXT("Factory class '%s' does not expose async-action factory function '%s'."),
					*FactoryClass->GetPathName(),
					*FactoryFunctionName.ToString()),
				Node.Id);
			return false;
		}
		...
		if (!FactoryFunction->HasMetaData(TEXT("BlueprintInternalUseOnly")))
		{
			Context.AddDiagnostic(
				...
				FString::Printf(
					TEXT("Async-action factory function '%s::%s' must be marked BlueprintInternalUseOnly to lower through UK2Node_AsyncAction."),
					*FactoryClass->GetPathName(),
					*FactoryFunctionName.ToString()),
				Node.Id);
			return false;
	...
	bool ResolveSpecializedAsyncTaskFactory(
		const FVergilGraphNode& Node,
		const FVergilSpecializedAsyncTaskDescriptor& Descriptor,
		FVergilCompilerContext& Context,
		UFunction*& OutFactoryFunction,
		UClass*& OutProxyClass)
	{
		...
		FName FactoryFunctionName = NAME_None;
		UClass* ExpectedProxyClass = nullptr;

		if (Descriptor.Descriptor == AIMoveToDescriptor)
		{
			FactoryClass = UAIBlueprintHelperLibrary::StaticClass();
			FactoryFunctionName = GET_FUNCTION_NAME_CHECKED(UAIBlueprintHelperLibrary, CreateMoveToProxyObject);
			ExpectedProxyClass = UAIAsyncTaskBlueprintProxy::StaticClass();
		}
		else if (Descriptor.Descriptor == PlayMontageDescriptor)
		{
			FactoryClass = UPlayMontageCallbackProxy::StaticClass();
			FactoryFunctionName = GET_FUNCTION_NAME_CHECKED(UPlayMontageCallbackProxy, CreateProxyObjectForPlayMontage);
			ExpectedProxyClass = UPlayMontageCallbackProxy::StaticClass();
		}
		else
		{
			Context.AddDiagnostic(
				EVergilDiagnosticSeverity::Error,
				TEXT("UnsupportedSpecializedAsyncTaskDescriptor"),
				FString::Printf(TEXT("%s is not a recognized specialized async-task descriptor."), *Node.Descriptor.ToString()),
				Node.Id);
			return false;
		}

		UFunction* const FactoryFunction = FactoryClass != nullptr ? FactoryClass->FindFunctionByName(FactoryFunctionName) : nullptr;
		if (FactoryFunction == nullptr)
		{
			Context.AddDiagnostic(
				EVergilDiagnosticSeverity::Error,
				TEXT("SpecializedAsyncTaskFactoryFunctionNotFound"),
				FString::Printf(
					TEXT("%s requires factory function '%s::%s', but it was not found in UE_5.7."),
					*Descriptor.Descriptor.ToString(),
					FactoryClass != nullptr ? *FactoryClass->GetPathName() : TEXT("<unknown>"),
					*FactoryFunctionName.ToString()),
				Node.Id);
			return false;
		}
		...
		if (!FactoryFunction->HasAllFunctionFlags(FUNC_Static))
		{
			Context.AddDiagnostic(
				EVergilDiagnosticSeverity::Error,
				TEXT("SpecializedAsyncTaskFactoryFunctionNotStatic"),
				FString::Printf(
					TEXT("%s requires static factory function '%s::%s'."),
					*Descriptor.Descriptor.ToString(),
					*FactoryClass->GetPathName(),
					*FactoryFunctionName.ToString()),
				Node.Id);
		...
		if (!FactoryFunction->HasAllFunctionFlags(FUNC_BlueprintCallable))
		{
			Context.AddDiagnostic(
				EVergilDiagnosticSeverity::Error,
				TEXT("SpecializedAsyncTaskFactoryFunctionNotCallable"),
				FString::Printf(
					TEXT("%s requires BlueprintCallable factory function '%s::%s'."),
					*Descriptor.Descriptor.ToString(),
					*FactoryClass->GetPathName(),
					*FactoryFunctionName.ToString()),
				Node.Id);
		...
		if (!FactoryFunction->HasMetaData(TEXT("BlueprintInternalUseOnly")))
		{
			Context.AddDiagnostic(
				EVergilDiagnosticSeverity::Error,
				TEXT("SpecializedAsyncTaskFactoryFunctionNotBlueprintInternal"),
				FString::Printf(
					TEXT("%s requires BlueprintInternalUseOnly factory function '%s::%s'."),
					*Descriptor.Descriptor.ToString(),
					*FactoryClass->GetPathName(),
					*FactoryFunctionName.ToString()),
				Node.Id);
		...
		if (ReturnProperty == nullptr || ReturnProperty->PropertyClass == nullptr)
		{
			Context.AddDiagnostic(
				EVergilDiagnosticSeverity::Error,
				TEXT("SpecializedAsyncTaskFactoryReturnInvalid"),
				FString::Printf(
					TEXT("%s requires factory function '%s::%s' to return an object proxy."),
					*Descriptor.Descriptor.ToString(),
					*FactoryClass->GetPathName(),
					*FactoryFunctionName.ToString()),
				Node.Id);
		...
		if (ExpectedProxyClass != nullptr && !ReturnProperty->PropertyClass->IsChildOf(ExpectedProxyClass))
		{
			Context.AddDiagnostic(
				EVergilDiagnosticSeverity::Error,
				TEXT("SpecializedAsyncTaskProxyClassInvalid"),
				FString::Printf(
					TEXT("%s requires factory function '%s::%s' to return '%s', but UE_5.7 exposes '%s'."),
					*Descriptor.Descriptor.ToString(),
					*FactoryClass->GetPathName(),
					*FactoryFunctionName.ToString(),
					*ExpectedProxyClass->GetPathName(),
					*ReturnProperty->PropertyClass->GetPathName()),
				Node.Id);
			return false;
		}
	...
	const FVergilComponentLookupDescriptor* FindComponentLookupDescriptorByDescriptor(const FName Descriptor)
	{
		static const FVergilComponentLookupDescriptor Descriptors[] =
		{
			{ TEXT("K2.GetComponentByClass"), TEXT("Vergil.K2.GetComponentByClass"), ActorGetComponentByClassFunctionName, false },
			{ TEXT("K2.GetComponentsByClass"), TEXT("Vergil.K2.GetComponentsByClass"), GET_FUNCTION_NAME_CHECKED(AActor, K2_GetComponentsByClass), false },
			{ TEXT("K2.FindComponentByTag"), TEXT("Vergil.K2.FindComponentByTag"), ActorFindComponentByTagFunctionName, true },
			{ TEXT("K2.GetComponentsByTag"), TEXT("Vergil.K2.GetComponentsByTag"), GET_FUNCTION_NAME_CHECKED(AActor, GetComponentsByTag), true },
		};
	...
	bool ValidateNodeSemanticRequirements(const FVergilGraphNode& Node, FVergilCompilerContext& Context)
	{
		...
		if (Node.Descriptor == TEXT("K2.AddComponentByClass"))
		{
			bIsValid &= HasRequiredMetadataValue(
				Node,
				TEXT("ComponentClassPath"),
				Context,
				TEXT("MissingAddComponentClassPath"),
				TEXT("AddComponentByClass nodes require metadata ComponentClassPath naming the component class to create."));
			return bIsValid;
		}

		if (const FVergilComponentLookupDescriptor* const ComponentLookupDescriptor = FindComponentLookupDescriptorByDescriptor(Node.Descriptor))
		{
			bIsValid &= HasRequiredMetadataValue(
				Node,
				TEXT("ComponentClassPath"),
		...
		if (Node.Descriptor.ToString().StartsWith(AsyncActionDescriptorPrefix))
		{
			bIsValid &= HasRequiredMetadataValue(
				Node,
				AsyncActionFactoryClassMetadataName,
				Context,
				TEXT("MissingAsyncActionFactoryClassPath"),
				TEXT("Async action nodes require metadata FactoryClassPath naming the factory-function owner class."));

			if (GetDescriptorSuffix(Node.Descriptor, AsyncActionDescriptorPrefix).IsEmpty())
			{
				AddNodeDiagnostic(
					TEXT("MissingAsyncActionFactoryFunction"),
					TEXT("Async action nodes require a descriptor suffix naming the factory function."));
			...
			if (Context.GetGraphName() == ConstructionScriptGraphName)
			{
				AddNodeDiagnostic(
					...
					TEXT("K2.AsyncAction.* is not supported when compiling UserConstructionScript because generic async-action nodes are latent."));
			}

			return bIsValid;
		}

		if (FindSpecializedAsyncTaskDescriptorByDescriptor(Node.Descriptor) != nullptr)
		{
			if (Context.GetGraphName() == ConstructionScriptGraphName)
			{
				AddNodeDiagnostic(
					TEXT("ConstructionScriptSpecializedAsyncTaskUnsupported"),
					FString::Printf(TEXT("%s is not supported when compiling UserConstructionScript because specialized async-task nodes are latent."), *Node.Descriptor.ToString()));
			}
	...
	bool ResolveAndNormalizeActorComponentClassMetadata(
		FVergilGraphNode& Node,
		const FName MetadataKey,
		...
		UClass*& OutComponentClass)
	{
		...
		const FString ComponentClassPath = Node.Metadata.FindRef(MetadataKey).TrimStartAndEnd();
		FString NormalizedClassPath;
		...
		if (ComponentClass == nullptr || !ComponentClass->IsChildOf(UActorComponent::StaticClass()))
		{
			AddTypeResolutionDiagnostic(
				...
				FString::Printf(TEXT("%s '%s' must resolve to a UActorComponent-derived class."), ContextLabel, *NormalizedClassPath),
				Node.Id);
			return false;
		}

		bOutChanged |= SetNormalizedMetadataValue(Node.Metadata, MetadataKey, NormalizedClassPath);
		OutComponentClass = ComponentClass;
		return true;
	}
	...
	{
		OutPins.Reset();
		AddUniqueSupportedPin(OutPins, UEdGraphSchema_K2::PN_Execute, EVergilPinDirection::Input, true);

		const bool bHideThen = DoesAsyncTaskHideThen(ProxyClass);
		if (!bHideThen)
		{
			AddUniqueSupportedPin(OutPins, UEdGraphSchema_K2::PN_Then, EVergilPinDirection::Output, true);
		}

		if (DoesAsyncTaskExposeProxy(ProxyClass))
		{
			AddUniqueSupportedPin(OutPins, AsyncActionProxyPinName, EVergilPinDirection::Output, false);
		}
		...
		if (!bHideThen && FactoryFunction != nullptr)
		{
			for (TFieldIterator<FProperty> PropertyIt(FactoryFunction); PropertyIt && (PropertyIt->PropertyFlags & CPF_Parm); ++PropertyIt)
			{
				...
				{
					AddUniqueSupportedPin(OutPins, Parameter->GetFName(), EVergilPinDirection::Output, false);
				}
			}
		}
		...
		for (TFieldIterator<FProperty> PropertyIt(ProxyClass); PropertyIt; ++PropertyIt)
		{
			...
			{
				AddUniqueSupportedPin(OutPins, DelegateProperty->GetFName(), EVergilPinDirection::Output, true);
		...
		if (FactoryFunction != nullptr)
		{
			...
			GatherHiddenAsyncTaskPins(FactoryFunction, HiddenPins);
			for (TFieldIterator<FProperty> PropertyIt(FactoryFunction); PropertyIt && (PropertyIt->PropertyFlags & CPF_Parm); ++PropertyIt)
			{
				FProperty* const Parameter = *PropertyIt;
				if (Parameter == nullptr)
				...
				{
					AddUniqueSupportedPin(OutPins, Parameter->GetFName(), EVergilPinDirection::Input, false);
				}
			}
		}
	}

	bool ValidateAsyncActionPins(
		const FVergilGraphNode& Node,
		const UFunction* FactoryFunction,
		const UClass* ProxyClass,
		FVergilCompilerContext& Context)
	{
		...
		for (const FVergilGraphPin& Pin : Node.Pins)
		{
			...
			const bool bTargetsHiddenPin = HiddenPins.Contains(Pin.Name);
			Context.AddDiagnostic(
				EVergilDiagnosticSeverity::Error,
				bTargetsHiddenPin ? TEXT("AsyncActionHiddenPinUnsupported") : TEXT("AsyncActionPinUnsupported"),
				bTargetsHiddenPin
					? FString::Printf(
						TEXT("%s pin '%s' is hidden by the UE_5.7 async-action factory surface and is not part of the authored deterministic contract."),
						*Node.Descriptor.ToString(),
						*Pin.Name.ToString())
					: FString::Printf(
						TEXT("%s pin '%s' is not part of the deterministic UE_5.7 async-action surface for factory '%s::%s'."),
						*Node.Descriptor.ToString(),
						*Pin.Name.ToString(),
	...
	bool ValidateSpecializedAsyncTaskPins(
		const FVergilGraphNode& Node,
		const FVergilSpecializedAsyncTaskDescriptor& Descriptor,
		const UFunction* FactoryFunction,
		const UClass* ProxyClass,
		FVergilCompilerContext& Context)
	{
		...
		for (const FVergilGraphPin& Pin : Node.Pins)
		{
			...
			Context.AddDiagnostic(
				EVergilDiagnosticSeverity::Error,
				bTargetsHiddenPin ? TEXT("SpecializedAsyncTaskHiddenPinUnsupported") : TEXT("SpecializedAsyncTaskPinUnsupported"),
				bTargetsHiddenPin
					? FString::Printf(
						TEXT("%s pin '%s' is hidden by the UE_5.7 specialized async-task surface and is not part of the authored deterministic contract."),
						*Descriptor.Descriptor.ToString(),
						*Pin.Name.ToString())
					: FString::Printf(
						TEXT("%s pin '%s' is not part of the deterministic UE_5.7 specialized async-task surface for factory '%s::%s'."),
						*Descriptor.Descriptor.ToString(),
						*Pin.Name.ToString(),
						FactoryFunction != nullptr && FactoryFunction->GetOwnerClass() != nullptr ? *FactoryFunction->GetOwnerClass()->GetPathName() : TEXT("<unknown>"),
						FactoryFunction != nullptr ? *FactoryFunction->GetFName().ToString() : TEXT("<unknown>")),
	...
	bool ResolveAndNormalizeAsyncActionNodeType(FVergilGraphNode& Node, FVergilCompilerContext& Context, bool& bOutChanged)
	{
		UClass* FactoryClass = nullptr;
		if (!ResolveAndNormalizeClassMetadata(
			Node,
			...
			TEXT("async-action factory class"),
			Context,
			bOutChanged,
			FactoryClass))
		{
		...
		const FString FactoryFunctionNameString = GetDescriptorSuffix(Node.Descriptor, AsyncActionDescriptorPrefix);
		if (FactoryFunctionNameString.IsEmpty())
		{
			Context.AddDiagnostic(
				EVergilDiagnosticSeverity::Error,
				TEXT("MissingAsyncActionFactoryFunction"),
		...
		if (!ResolveAsyncActionFactory(Node, FactoryClass, *FactoryFunctionNameString, Context, FactoryFunction, ProxyClass))
		{
			return false;
		}

		return ValidateAsyncActionPins(Node, FactoryFunction, ProxyClass, Context);
	}

	bool ResolveAndNormalizeSpecializedAsyncTaskNodeType(
		FVergilGraphNode& Node,
		const FVergilSpecializedAsyncTaskDescriptor& Descriptor,
		FVergilCompilerContext& Context,
		bool& bOutChanged)
	{
		...
		UFunction* FactoryFunction = nullptr;
		UClass* ProxyClass = nullptr;
		if (!ResolveSpecializedAsyncTaskFactory(Node, Descriptor, Context, FactoryFunction, ProxyClass))
		{
			return false;
		}

		return ValidateSpecializedAsyncTaskPins(Node, Descriptor, FactoryFunction, ProxyClass, Context);
	}
	...
	void BuildAddComponentSupportedPins(const UClass* ComponentClass, TArray<FVergilSupportedPinDescriptor>& OutPins)
	{
		...
		OutPins.Add({ UEdGraphSchema_K2::PN_Self, EVergilPinDirection::Input, false });
		...
		{
			OutPins.Add({ AddComponentManualAttachmentPinName, EVergilPinDirection::Input, false });
			OutPins.Add({ AddComponentRelativeTransformPinName, EVergilPinDirection::Input, false });
		}
		...
		FVergilCompilerContext& Context)
	{
		...
		for (const FVergilGraphPin& Pin : Node.Pins)
		{
			if (Pin.Name == AddComponentClassPinName)
			{
				Context.AddDiagnostic(
					EVergilDiagnosticSeverity::Error,
					TEXT("AddComponentDynamicClassPinUnsupported"),
					TEXT("K2.AddComponentByClass currently uses ComponentClassPath metadata as its deterministic class source; authored Class-pin connections are not supported."),
					Node.Id);
				bIsValid = false;
				continue;
			}
			...
			if (SupportedPin == nullptr)
			{
				Context.AddDiagnostic(
					...
					FString::Printf(
						TEXT("K2.AddComponentByClass pin '%s' is not part of the UE_5.7 AddComponentByClass surface for component class '%s'."),
						*Pin.Name.ToString(),
	...
	void BuildComponentLookupSupportedPins(const bool bSupportsTag, TArray<FVergilSupportedPinDescriptor>& OutPins)
	{
		...
		if (bSupportsTag)
		{
			OutPins.Add({ ComponentLookupTagPinName, EVergilPinDirection::Input, false });
		}
	}

	bool ValidateComponentLookupPins(
		const FVergilGraphNode& Node,
		const FVergilComponentLookupDescriptor& Descriptor,
		const UClass* ComponentClass,
		FVergilCompilerContext& Context)
	{
		TArray<FVergilSupportedPinDescriptor> SupportedPins;
		BuildComponentLookupSupportedPins(Descriptor.bSupportsTag, SupportedPins);

		bool bIsValid = true;
		for (const FVergilGraphPin& Pin : Node.Pins)
		{
			if (Pin.Name == ComponentLookupClassPinName)
			{
				Context.AddDiagnostic(
					EVergilDiagnosticSeverity::Error,
					TEXT("ComponentLookupDynamicClassPinUnsupported"),
					FString::Printf(
						TEXT("%s currently uses ComponentClassPath metadata as its deterministic class source; authored ComponentClass pin connections are not supported."),
						*Descriptor.Descriptor.ToString()),
					Node.Id);
				bIsValid = false;
				continue;
	...
	bool ResolveAndNormalizeAddComponentByClassNodeType(FVergilGraphNode& Node, FVergilCompilerContext& Context, bool& bOutChanged)
	{
		UClass* ComponentClass = nullptr;
		if (!ResolveAndNormalizeActorComponentClassMetadata(
			Node,
			TEXT("ComponentClassPath"),
			TEXT("AddComponentClassNotFound"),
			TEXT("AddComponentClassNotComponent"),
			TEXT("add-component class"),
			Context,
			bOutChanged,
			ComponentClass))
		{
		...
		return ValidateAddComponentPins(Node, ComponentClass, Context);
	}

	bool ResolveAndNormalizeComponentLookupNodeType(FVergilGraphNode& Node, FVergilCompilerContext& Context, bool& bOutChanged)
	{
		const FVergilComponentLookupDescriptor* const Descriptor = FindComponentLookupDescriptorByDescriptor(Node.Descriptor);
	...
	bool ResolveNodeTypes(FVergilGraphNode& Node, FVergilCompilerContext& Context, bool& bOutChanged)
	{
		if (Node.Descriptor == TEXT("K2.AddComponentByClass"))
		{
			return ResolveAndNormalizeAddComponentByClassNodeType(Node, Context, bOutChanged);
		}
		...
		if (Node.Descriptor.ToString().StartsWith(AsyncActionDescriptorPrefix))
		{
			return ResolveAndNormalizeAsyncActionNodeType(Node, Context, bOutChanged);
		}

		if (const FVergilSpecializedAsyncTaskDescriptor* const SpecializedAsyncTaskDescriptor = FindSpecializedAsyncTaskDescriptorByDescriptor(Node.Descriptor))
		{
			return ResolveAndNormalizeSpecializedAsyncTaskNodeType(Node, *SpecializedAsyncTaskDescriptor, Context, bOutChanged);
		}
	...
	class FVergilAsyncActionNodeHandler final : public IVergilNodeHandler
	{
	public:
		virtual FName GetDescriptor() const override
		{
			return AsyncActionCommandName;
		}

		virtual bool CanHandle(const FVergilGraphNode& Node) const override
		{
			return Node.Descriptor.ToString().StartsWith(AsyncActionDescriptorPrefix);
		}

		virtual bool BuildCommands(const FVergilGraphNode& Node, FVergilCompilerContext& Context) const override
		{
			const FString FactoryFunctionName = GetDescriptorSuffix(Node.Descriptor, AsyncActionDescriptorPrefix);
			...
			const FString FactoryClassPath = Node.Metadata.FindRef(AsyncActionFactoryClassMetadataName);
			...
			Command.NodeId = Node.Id;
			Command.Name = GetDescriptor();
			Command.SecondaryName = *FactoryFunctionName;
			Command.StringValue = FactoryClassPath;
			Command.Position = Node.Position;
			Command.Attributes = Node.Metadata;
			CopyPlannedPins(Node, Command);
			Context.AddCommand(Command);
	...
	class FVergilSpecializedAsyncTaskNodeHandler final : public IVergilNodeHandler
	{
	public:
		explicit FVergilSpecializedAsyncTaskNodeHandler(const FVergilSpecializedAsyncTaskDescriptor& InDescriptor)
			: Descriptor(InDescriptor)
		{
		}
	...
	private:
		FVergilSpecializedAsyncTaskDescriptor Descriptor;
	};
	...
	class FVergilAddComponentByClassNodeHandler final : public IVergilNodeHandler
	{
	public:
		virtual FName GetDescriptor() const override
		{
			return TEXT("Vergil.K2.AddComponentByClass");
		}

		virtual bool CanHandle(const FVergilGraphNode& Node) const override
		{
			return Node.Descriptor == TEXT("K2.AddComponentByClass");
		}

		virtual bool BuildCommands(const FVergilGraphNode& Node, FVergilCompilerContext& Context) const override
		{
			const FString ComponentClassPath = Node.Metadata.FindRef(TEXT("ComponentClassPath"));
			if (ComponentClassPath.IsEmpty())
			{
				Context.AddDiagnostic(EVergilDiagnosticSeverity::Error, TEXT("MissingAddComponentClassPath"), TEXT("AddComponentByClass nodes require metadata ComponentClassPath."), Node.Id);
				return false;
			}
			...
			Command.NodeId = Node.Id;
			Command.Name = GetDescriptor();
			Command.StringValue = ComponentClassPath;
			Command.Position = Node.Position;
			Command.Attributes = Node.Metadata;
			CopyPlannedPins(Node, Command);
			Context.AddCommand(Command);
	...
	class FVergilComponentLookupNodeHandler final : public IVergilNodeHandler
	{
	public:
		explicit FVergilComponentLookupNodeHandler(const FVergilComponentLookupDescriptor& InDescriptor)
			: Descriptor(InDescriptor)
		{
		}
		...
		virtual bool BuildCommands(const FVergilGraphNode& Node, FVergilCompilerContext& Context) const override
		{
			...
			if (ComponentClassPath.IsEmpty())
			{
				Context.AddDiagnostic(EVergilDiagnosticSeverity::Error, TEXT("MissingComponentLookupClassPath"), TEXT("Component lookup nodes require metadata ComponentClassPath."), Node.Id);
				return false;
			}
	...
	private:
		FVergilComponentLookupDescriptor Descriptor;
	};
	...
	void EnsureGenericFallbackHandler()
	{
		...
		FVergilNodeRegistry::Get().RegisterFallbackHandler(MakeShared<FVergilAsyncActionNodeHandler, ESPMode::ThreadSafe>());
		if (const FVergilSpecializedAsyncTaskDescriptor* const AIMoveToTaskDescriptor = FindSpecializedAsyncTaskDescriptorByDescriptor(AIMoveToDescriptor))
		{
			FVergilNodeRegistry::Get().RegisterFallbackHandler(MakeShared<FVergilSpecializedAsyncTaskNodeHandler, ESPMode::ThreadSafe>(*AIMoveToTaskDescriptor));
		}
		if (const FVergilSpecializedAsyncTaskDescriptor* const PlayMontageTaskDescriptor = FindSpecializedAsyncTaskDescriptorByDescriptor(PlayMontageDescriptor))
		{
			FVergilNodeRegistry::Get().RegisterFallbackHandler(MakeShared<FVergilSpecializedAsyncTaskNodeHandler, ESPMode::ThreadSafe>(*PlayMontageTaskDescriptor));
		}
		FVergilNodeRegistry::Get().RegisterFallbackHandler(MakeShared<FVergilSpawnActorNodeHandler, ESPMode::ThreadSafe>());
		FVergilNodeRegistry::Get().RegisterFallbackHandler(MakeShared<FVergilAddComponentByClassNodeHandler, ESPMode::ThreadSafe>());
		if (const FVergilComponentLookupDescriptor* const GetComponentByClassDescriptor = FindComponentLookupDescriptorByDescriptor(TEXT("K2.GetComponentByClass")))
		{
			FVergilNodeRegistry::Get().RegisterFallbackHandler(MakeShared<FVergilComponentLookupNodeHandler, ESPMode::ThreadSafe>(*GetComponentByClassDescriptor));
		}
		if (const FVergilComponentLookupDescriptor* const GetComponentsByClassDescriptor = FindComponentLookupDescriptorByDescriptor(TEXT("K2.GetComponentsByClass")))
		{
			FVergilNodeRegistry::Get().RegisterFallbackHandler(MakeShared<FVergilComponentLookupNodeHandler, ESPMode::ThreadSafe>(*GetComponentsByClassDescriptor));
		}
		if (const FVergilComponentLookupDescriptor* const FindComponentByTagDescriptor = FindComponentLookupDescriptorByDescriptor(TEXT("K2.FindComponentByTag")))
		{
			FVergilNodeRegistry::Get().RegisterFallbackHandler(MakeShared<FVergilComponentLookupNodeHandler, ESPMode::ThreadSafe>(*FindComponentByTagDescriptor));
		}
		if (const FVergilComponentLookupDescriptor* const GetComponentsByTagDescriptor = FindComponentLookupDescriptorByDescriptor(TEXT("K2.GetComponentsByTag")))
		{
			FVergilNodeRegistry::Get().RegisterFallbackHandler(MakeShared<FVergilComponentLookupNodeHandler, ESPMode::ThreadSafe>(*GetComponentsByTagDescriptor));
		}
