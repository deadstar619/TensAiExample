	...
	TArray<FVergilSupportedDescriptorContract> BuildSupportedDescriptorContracts()
	{
		...
		return {
			...
			MakeDescriptorContract(
				...
				TEXT("ComponentClassPath must resolve to a UActorComponent-derived class during type resolution and is normalized before planning. Under UE_5.7 this lowers to UK2Node_AddComponentByClass with the fixed deterministic surface: Self, ReturnValue, scene-component-only bManualAttachment and RelativeTransform pins, and class-specific ExposeOnSpawn property pins. The dynamic Class pin is intentionally not part of the authored deterministic contract.")),
			MakeDescriptorContract(
