	...
	TArray<FVergilSupportedDescriptorContract> BuildSupportedDescriptorContracts()
	{
		...
		return {
			...
			MakeDescriptorContract(
				...
				TEXT("Optional IndexObjectPath, ValueObjectPath, and NumOptions metadata refine the wildcard shape. Enum index selects use IndexObjectPath for the enum, and explicit type metadata is resolved before planning.")),
			MakeDescriptorContract(
				TEXT("K2.SwitchInt"),
				EVergilDescriptorMatchKind::Exact,
				TEXT("any"),
				SupportedGraphs,
				{},
				TEXT("Planned exec output pins define the case labels and must parse as integers.")),
			MakeDescriptorContract(
				...
				SupportedGraphs,
				{},
				TEXT("Planned exec output pins define the case labels. Optional CaseSensitive metadata configures comparison behavior.")),
			MakeDescriptorContract(
				TEXT("K2.SwitchEnum"),
				EVergilDescriptorMatchKind::Exact,
				TEXT("any"),
				SupportedGraphs,
				{ TEXT("EnumPath") },
				TEXT("EnumPath must resolve to a UEnum during type resolution and is normalized before planning.")),
			MakeDescriptorContract(
				TEXT("K2.FormatText"),
				EVergilDescriptorMatchKind::Exact,
				TEXT("any"),
