...
- `FVergilCompileRequest.TargetGraphName` selects one graph per compile. The request type defaults to `EventGraph`, while `UVergilEditorSubsystem` and `UVergilAgentSubsystem` now seed omitted graph targets from `UVergilDeveloperSettings::DefaultTargetGraphName` before falling back to `EventGraph`. The primary graph surface still uses top-level `Nodes` and `Edges`, while `ConstructionScriptNodes` and `ConstructionScriptEdges` are the dedicated document surface for `UserConstructionScript`.
...
- Construction script definitions now lower into construction-script graph authoring when `FVergilCompileRequest.TargetGraphName` is `UserConstructionScript`. `UVergilEditorSubsystem::CompileDocument(...)` and `PreviewDocument(...)` now use the configured `UVergilDeveloperSettings::DefaultTargetGraphName` when no explicit graph is passed, and `CompileDocumentToGraph(..., UserConstructionScript, ...)` remains the explicit per-call override when the caller wants to bypass that project default.
...
- `FVergilCompileRequest.bGenerateComments` now controls whether authored comment nodes are emitted through the dedicated comment post-pass, and `FVergilCompileRequest.CommentGeneration` now carries explicit `UE_5.7`-backed defaults for comment width, height, font size, title color, zoom bubble visibility, bubble coloring, and move mode. `bAutoLayout` plus `AutoLayout` still control deterministic `MoveNode` generation through the dedicated layout post-pass. `AutoLayout` currently supports `Origin`, `HorizontalSpacing`, `VerticalSpacing`, and `CommentPadding`, and `UVergilDeveloperSettings` now carries the full default target-graph, auto-layout, comment-generation, and structural-warning strictness surface used to seed editor-facing requests.
- `FVergilCompileRequest.bTreatStructuralWarningsAsErrors` now promotes structural-validation warnings such as missing authored component parents to errors before semantic validation when enabled, giving teams a stricter project-level validation bar without disabling any existing safety checks.
- `UVergilEditorSubsystem::MakeDefaultCompileRequest(...)` returns a request seeded entirely from `UVergilDeveloperSettings`, while `MakeCompileRequest(...)` preserves the same seeded nested settings but lets callers override the target graph plus auto-layout/comment-generation enabled flags per call. `CompileRequest(...)` lets callers run that explicit request, and `PreviewCompileRequest(...)` / `PreviewDocument(...)` expose the same dry-run path through a dedicated preview payload instead of forcing tooling to reconstruct it from logs.
- `FVergilCompileResult` now includes `Statistics` plus ordered `PassRecords`. Compiler-produced results report target graph, requested/effective schema version, requested auto-layout/comment flags, normalized per-phase command counts, plan fingerprints, planning/apply invocation counts, apply-side undo/redo transaction audits including partial-apply recovery outcomes, and the last completed or failed compiler pass without requiring log scraping.
...
- Deterministic component-related node support now exists for `K2.AddComponentByClass`, `K2.GetComponentByClass`, `K2.GetComponentsByClass`, `K2.FindComponentByTag`, and `K2.GetComponentsByTag`. Each requires `ComponentClassPath`; the dynamic `Class` and `ComponentClass` pins are intentionally outside the authored contract surface.
- Deterministic Blueprint-interface invocation support now exists for `K2.InterfaceCall.*` and `K2.InterfaceMessage.*`. Each requires `InterfaceClassPath`; direct calls lower to the real interface `UK2Node_CallFunction` surface, while messages lower to `UK2Node_Message` and keep the engine's object-targeted no-op-if-unimplemented behavior.
- Deterministic generic Blueprint async-action support now exists for `K2.AsyncAction.*`. Each requires `FactoryClassPath`, the descriptor suffix names the static `BlueprintInternalUseOnly` factory function, and hidden pins such as `WorldContextObject` remain outside the authored contract surface.
- Deterministic specialized async-task support now also exists for `K2.AIMoveTo` and `K2.PlayMontage`, each lowering through a dedicated handler to the real `UE_5.7` node class instead of forcing those `HasDedicatedAsyncNode` families through the generic async-action path.
...
- The real agent-layer contract is now explicit and versioned. `PlanDocument` requests wrap `FVergilGraphDocument` plus target graph and compile flags, `ApplyCommandPlan` requests wrap explicit command plans plus target Blueprint path and expected fingerprint, `FVergilAgentRequestContext` carries request/session/parent correlation ids plus write-approval metadata for apply replay, responses wrap `FVergilCompileResult`, audit entries store the request/response pair plus a UTC timestamp, and the persisted audit-log wrapper stores those normalized entries on disk.
- `UVergilAgentSubsystem::ExecuteRequest(...)` is the supported orchestration entry point for those agent requests. Planning stays read-only, apply requests replay only the explicit provided command plan, failed apply requests now auto-recover their own recorded Vergil transaction when that rollback is safe, missing request/session provenance is normalized before execution, and each execution appends an audit entry automatically.
...
- The document-diff inspection format is `format="Vergil.DocumentDiff"` with `version=1`, sourced from `Vergil::GetDocumentDiffInspectionFormatName()` and `Vergil::GetDocumentDiffInspectionFormatVersion()`.
...
- The agent request format is `format="Vergil.AgentRequest"` with `version=3`, sourced from `Vergil::GetAgentRequestFormatName()` and `Vergil::GetAgentRequestFormatVersion()`.
- The agent response format is `format="Vergil.AgentResponse"` with `version=2`, sourced from `Vergil::GetAgentResponseFormatName()` and `Vergil::GetAgentResponseFormatVersion()`.
- The agent audit-entry format is `format="Vergil.AgentAuditEntry"` with `version=2`, sourced from `Vergil::GetAgentAuditEntryFormatName()` and `Vergil::GetAgentAuditEntryFormatVersion()`.
...
- `Vergil::DescribeGraphDocument(...)`, `Vergil::DescribeDocumentDiff(...)`, `Vergil::DescribeDiagnostics(...)`, `Vergil::DescribeCommandPlan(...)`, `Vergil::DescribeCompileResult(...)`, and `Vergil::DescribeCommandPlanPreview(...)` return stable human-readable inspection strings intended for tooling, logs, and previews.
- `Vergil::SerializeGraphDocument(...)`, `Vergil::SerializeDocumentDiff(...)`, `Vergil::SerializeDiagnostics(...)`, `Vergil::SerializeCompileResult(...)`, and `Vergil::SerializeCommandPlanPreview(...)` return deterministic JSON inspection payloads. `Vergil.CommandPlanPreview` nests the current graph-document, document-diff, and compile-result payloads so callers can inspect the effective working document plus planned mutations without a separate scrape step.
- `Vergil::InspectReflectionSymbol(...)` resolves classes, structs, enums, and Blueprint generated classes from explicit script/object/package references plus unique short names, while `Vergil::DiscoverReflectionSymbols(...)` searches the currently loaded reflected type set by name or path fragment.
- `Vergil::DescribeReflectionSymbol(...)` / `SerializeReflectionSymbol(...)` and `Vergil::DescribeReflectionDiscovery(...)` / `SerializeReflectionDiscovery(...)` provide the stable reflection inspection/discovery surface for those resolved symbols and search results.
- `UVergilEditorSubsystem` exposes `PreviewCompileRequest()`, `PreviewDocument()`, `PreviewDocumentToGraph()`, `DescribeCommandPlan()`, `SerializeCommandPlan()`, `DescribeDocument()`, `SerializeDocument()`, `DiffDocuments()`, `DescribeDocumentDiff()`, `InspectDocumentDiffAsJson()`, `DescribeDiagnostics()`, `SerializeDiagnostics()`, `DescribeCompileResult()`, `SerializeCompileResult()`, `DescribeCommandPlanPreview()`, and `InspectCommandPlanPreviewAsJson()` as the supported editor-facing inspection and preview surface.
- `UVergilEditorSubsystem` also exposes `InspectReflectionSymbol()`, `DescribeReflectionSymbol()`, `InspectReflectionSymbolAsJson()`, `DiscoverReflectionSymbols()`, `DescribeReflectionDiscovery()`, and `InspectReflectionDiscoveryAsJson()` for bridge-friendly runtime/reflection inspection.
- `UVergilAgentSubsystem` mirrors those read-only helpers through `DescribeCommandPlan()`, `InspectCommandPlanAsJson()`, `DescribeDocument()`, `InspectDocumentAsJson()`, `DiffDocuments()`, `DescribeDocumentDiff()`, `InspectDocumentDiffAsJson()`, `DescribeDiagnostics()`, `InspectDiagnosticsAsJson()`, `DescribeCompileResult()`, `InspectCompileResultAsJson()`, `DescribeCommandPlanPreview()`, and `InspectCommandPlanPreviewAsJson()`, plus `InspectReflectionSymbol()`, `DescribeReflectionSymbol()`, `InspectReflectionSymbolAsJson()`, `DiscoverReflectionSymbols()`, `DescribeReflectionDiscovery()`, and `InspectReflectionDiscoveryAsJson()`.
...
- `FVergilAgentRequestContext` currently carries `RequestId`, `SessionId`, `ParentRequestId`, `Summary`, `InputText`, `Tags`, and `WriteAuthorization`.
...
- `FVergilAgentResponse` currently carries `RequestId`, `SessionId`, `ParentRequestId`, `Operation`, `State`, `Message`, and `Result`. `Result` stays the existing `FVergilCompileResult` so agent orchestration reuses the same compile/apply diagnostics, plan statistics, and normalized command-plan payload already documented elsewhere.
- `FVergilAgentAuditEntry` currently carries `Request`, `Response`, and `TimestampUtc`.
- `UVergilAgentSubsystem::MakeApplyRequestFromPlan(...)` is the supported helper for the explicit phase handoff. It copies the reviewed normalized plan, carries the target Blueprint path forward from the plan request, stamps the expected normalized command-plan fingerprint onto the apply request, inherits the plan session id when the apply context omits one, and stamps the plan request id as the apply parent request id when omitted.
...
- `UVergilAgentSubsystem::RecordAuditEntry(...)` now normalizes missing response request ids, missing response/session/parent correlation ids, missing response operations, and missing `TimestampUtc` before appending the audit trail and immediately persisting it.
...
- `FVergilCompileResult.Statistics.TransactionAudit` is populated only for apply paths. It records whether Vergil opened a scoped editor transaction, the explicit Vergil transaction context/title/primary Blueprint object, the before/failure/after undo-stack snapshots including queue length, undo count, next undo/redo transaction ids, titles, contexts, primary objects, and whether the target Blueprint is referenced by the undo buffer, plus whether failed apply recovery was required, attempted, and successful.
...
- The symbol pass resolves callable/member references for `K2.Event.*`, `K2.Call.*`, `K2.InterfaceCall.*`, `K2.InterfaceMessage.*`, `K2.VarGet.*`, `K2.VarSet.*`, `K2.BindDelegate.*`, `K2.RemoveDelegate.*`, `K2.ClearDelegate.*`, `K2.CallDelegate.*`, `K2.CreateDelegate.*`, custom-event delegate-signature metadata, and `K2.ForLoop`, `K2.ForLoopWithBreak`, `K2.DoOnce`, `K2.FlipFlop`, `K2.Gate`, and `K2.WhileLoop` macro references.
...
- `K2.ForLoop`, `K2.ForLoopWithBreak`, `K2.DoOnce`, `K2.FlipFlop`, `K2.Gate`, and `K2.WhileLoop` macro references now resolve during the symbol pass, and omitted `MacroBlueprintPath` / `MacroGraphName` metadata is normalized to the matching engine `StandardMacros` graph before planning.
...
- The type pass currently resolves explicit type metadata for `K2.Cast`, `K2.ClassCast`, `K2.GetClassDefaults`, `K2.AsyncAction.*`, `K2.AIMoveTo`, `K2.PlayMontage`, `K2.LoadAsset`, `K2.LoadAssetClass`, `K2.LoadAssets`, `K2.SpawnActor`, `K2.AddComponentByClass`, `K2.GetComponentByClass`, `K2.GetComponentsByClass`, `K2.FindComponentByTag`, `K2.GetComponentsByTag`, `K2.Select`, `K2.SwitchEnum`, `K2.MakeStruct`, `K2.BreakStruct`, `K2.MakeArray`, `K2.MakeSet`, and `K2.MakeMap`.
...
This table is mirrored from the code-backed supported-contract manifest. `Vergil.Scaffold.SupportedNodeContractDocs` fails if the checked-in markdown drifts from `Vergil::DescribeSupportedDescriptorContractsAsMarkdownTable()`.

<!-- BEGIN GENERATED SUPPORTED NODE CONTRACT TABLE -->
| Descriptor contract | Match | Expected kind | Target graphs | Required metadata | Notes |
| --- | --- | --- | --- | --- | --- |
| `any non-empty descriptor` | `NodeKind` | `Comment` | `EventGraph`, `UserConstructionScript` | `none` | Comment nodes are matched by kind, not a fixed descriptor string. CommentText or Title sets the text. CommentWidth, CommentHeight, FontSize, Color, CommentColor, ShowBubbleWhenZoomed, ColorBubble, and MoveMode are supported. When those style keys are omitted, the dedicated comment post-pass fills deterministic defaults from FVergilCompileRequest.CommentGeneration using UE_5.7 comment-node defaults as the baseline. |
| `K2.Event.<FunctionName>` | `Prefix` | `Event` | `EventGraph`, `UserConstructionScript` | `none` | Binds a standard event by function-name suffix. When targeting UserConstructionScript, the only supported event descriptor is K2.Event.UserConstructionScript. |
| `K2.CustomEvent.<EventName>` | `Prefix` | `any` | `EventGraph`, `UserConstructionScript` | `none` | Creates a custom event named by descriptor suffix. Optional DelegatePropertyName and DelegateOwnerClassPath metadata is resolved during compilation when authored. |
| `K2.Call.<FunctionName>` | `Prefix` | `Call` | `EventGraph`, `UserConstructionScript` | `none` | Optional OwnerClassPath constrains function resolution. When omitted, the current scaffold resolves document-authored functions first, then existing Blueprint-local functions, then inherited/native functions. Self-owned resolutions keep an empty owner path; inherited/native resolutions normalize the owner path into the planned command. Headless UE_5.7 coverage now explicitly re-verifies timer-by-function-name helpers plus handle-based timer pause/query helpers through this generic call path. |
| `K2.InterfaceCall.<FunctionName>` | `Prefix` | `Call` | `EventGraph`, `UserConstructionScript` | `InterfaceClassPath` | InterfaceClassPath must resolve to a UInterface-derived class and is normalized during symbol/type resolution. Under UE_5.7 this lowers to UK2Node_CallFunction bound against the interface function owner, preserving the interface-typed self pin instead of the generic object-target call surface. |
| `K2.InterfaceMessage.<FunctionName>` | `Prefix` | `Call` | `EventGraph`, `UserConstructionScript` | `InterfaceClassPath` | InterfaceClassPath must resolve to a UInterface-derived class and is normalized during symbol/type resolution. Under UE_5.7 this lowers to UK2Node_Message bound against the interface function owner, exposing the object-typed self/Target pin and the engine's no-op-if-unimplemented message semantics. |
| `K2.ClassCast` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `TargetClassPath` | TargetClassPath must resolve to a class during type resolution and is normalized before planning. Under UE_5.7 this lowers through the generic node-spawner path to UK2Node_ClassDynamicCast and preserves pure versus impure class-cast parity from the authored exec-pin surface. |
| `K2.GetClassDefaults` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `ClassPath` | ClassPath must resolve to a class during type resolution and is normalized before planning. Under UE_5.7 this lowers through the generic node-spawner path to UK2Node_GetClassDefaults with property-output pins sourced deterministically from the metadata-selected class; the dynamic Class pin is intentionally not part of the authored contract. |
| `K2.AsyncAction.<FactoryFunctionName>` | `Prefix` | `any` | `EventGraph` | `FactoryClassPath` | FactoryClassPath must resolve to the owner class of a static BlueprintInternalUseOnly factory function, and the descriptor suffix names that function. Under UE_5.7 this lowers through the generic node-spawner path to UK2Node_AsyncAction for generic UBlueprintAsyncActionBase factories that do not advertise HasDedicatedAsyncNode. The authored deterministic surface includes the visible factory input pins, Then/delegate exec outputs, and delegate payload outputs; hidden pins such as WorldContextObject remain engine-driven and are not part of the authored contract. |
| `K2.AIMoveTo` | `Exact` | `any` | `EventGraph` | `none` | Under UE_5.7 this resolves through the dedicated specialized async-task handler and then instantiates UK2Node_AIMoveTo through the generic node-spawner path, resolving UAIBlueprintHelperLibrary::CreateMoveToProxyObject instead of the generic async-action path. The authored deterministic surface includes the visible factory input pins plus Then, delegate exec outputs, and delegate payload outputs such as MovementResult; hidden pins such as WorldContextObject remain engine-driven and are not part of the authored contract. |
| `K2.PlayMontage` | `Exact` | `any` | `EventGraph` | `none` | Under UE_5.7 this resolves through the dedicated specialized async-task handler and then instantiates UK2Node_PlayMontage through the generic node-spawner path, resolving UPlayMontageCallbackProxy::CreateProxyObjectForPlayMontage instead of the generic async-action path. The authored deterministic surface includes the visible factory input pins plus Then, delegate exec outputs, and delegate payload outputs such as NotifyName. |
| `K2.LoadAsset` | `Exact` | `any` | `EventGraph` | `AssetClassPath` | AssetClassPath must resolve to a class during type resolution and is normalized before planning. Under UE_5.7 this lowers through the generic node-spawner path to UK2Node_LoadAsset with the latent Execute, Then, Completed, Asset, and Object surface. The Asset input pin is conformed to the metadata-selected soft-object type, while Object intentionally stays on the node's native UObject result family so the engine expansion remains compile-safe. |
| `K2.LoadAssetClass` | `Exact` | `any` | `EventGraph` | `AssetClassPath` | AssetClassPath must resolve to a class during type resolution and is normalized before planning. Under UE_5.7 this lowers through the generic node-spawner path to UK2Node_LoadAssetClass with the latent Execute, Then, Completed, AssetClass, and Class surface, and the input/output pins are conformed to the metadata-selected class type. |
| `K2.LoadAssets` | `Exact` | `any` | `EventGraph` | `AssetClassPath` | AssetClassPath must resolve to a class during type resolution and is normalized before planning. Under UE_5.7 this lowers through the generic node-spawner path to UK2Node_LoadAssets with the latent Execute, Then, Completed, Assets, and Objects surface. The Assets input array element type is conformed to the metadata-selected soft-object type, while Objects intentionally stays on the node's native UObject array-result family so the engine expansion remains compile-safe. |
| `K2.ConvertAsset` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `none` | Under UE_5.7 this lowers through the generic node-spawner path to UK2Node_ConvertAsset with the deterministic wildcard-free supported surface Input and Output. Type promotion comes from connected hard or soft object/class references rather than authored metadata. |
| `K2.VarGet.<VariableName>` | `Prefix` | `VariableGet` | `EventGraph`, `UserConstructionScript` | `none` | Optional OwnerClassPath constrains property lookup. Without it, the symbol pass resolves document-authored members first, then existing Blueprint members, then inherited members. UE_5.7 getter variants now support pure reads, bool branch getters, and validated object/class/soft-reference getters. Impure getter shapes must use Execute, Then, and Else exec pins. |
| `K2.VarSet.<VariableName>` | `Prefix` | `VariableSet` | `EventGraph`, `UserConstructionScript` | `none` | Optional OwnerClassPath constrains property lookup. Without it, the symbol pass resolves document-authored members first, then existing Blueprint members, then inherited members. |
| `K2.Self` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `none` | Creates a self node through the generic node-spawner path. |
| `K2.Branch` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `none` | Standard branch node instantiated through the generic node-spawner path. |
| `K2.Sequence` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `none` | Output exec pins named like Then_0, Then_1, and so on determine sequence width, and the backing UK2Node_ExecutionSequence is instantiated through the generic node-spawner path. |
| `K2.ForLoop` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `none` | Optional MacroBlueprintPath and MacroGraphName metadata selects the backing macro. Defaults resolve to the engine StandardMacros ForLoop and are validated before planning. |
| `K2.ForLoopWithBreak` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `none` | Optional MacroBlueprintPath and MacroGraphName metadata selects the backing macro. Defaults resolve to the engine StandardMacros ForLoopWithBreak and are validated before planning. |
| `K2.DoOnce` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `none` | Optional MacroBlueprintPath and MacroGraphName metadata selects the backing macro. Defaults resolve to the engine StandardMacros DoOnce and are validated before planning. The bool input follows the engine pin name 'Start Closed'. |
| `K2.FlipFlop` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `none` | Optional MacroBlueprintPath and MacroGraphName metadata selects the backing macro. Defaults resolve to the engine StandardMacros FlipFlop and are validated before planning. The input exec pin maps to the engine macro's unnamed entry exec pin, while outputs remain A, B, and IsA. |
| `K2.Gate` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `none` | Optional MacroBlueprintPath and MacroGraphName metadata selects the backing macro. Defaults resolve to the engine StandardMacros Gate and are validated before planning. The primary exec input follows the engine name 'Enter', and authored StartClosed pins normalize to the engine bool input 'bStartClosed'. |
| `K2.WhileLoop` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `none` | Optional MacroBlueprintPath and MacroGraphName metadata selects the backing macro. Defaults resolve to the engine StandardMacros WhileLoop and are validated before planning. |
| `K2.AddComponentByClass` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `ComponentClassPath` | ComponentClassPath must resolve to a UActorComponent-derived class during type resolution and is normalized before planning. Under UE_5.7 this lowers through the generic node-spawner path to UK2Node_AddComponentByClass with the fixed deterministic surface: Self, ReturnValue, scene-component-only bManualAttachment and RelativeTransform pins, and class-specific ExposeOnSpawn property pins. The dynamic Class pin is intentionally not part of the authored deterministic contract. |
| `K2.GetComponentByClass` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `ComponentClassPath` | ComponentClassPath must resolve to a UActorComponent-derived class during type resolution and is normalized before planning. Under UE_5.7 this lowers to AActor::GetComponentByClass with the return pin conformed from the metadata-driven ComponentClass input. The dynamic ComponentClass pin is intentionally not part of the authored deterministic contract. |
| `K2.GetComponentsByClass` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `ComponentClassPath` | ComponentClassPath must resolve to a UActorComponent-derived class during type resolution and is normalized before planning. Under UE_5.7 this lowers to AActor::K2_GetComponentsByClass with the array element type conformed from the metadata-driven ComponentClass input. The dynamic ComponentClass pin is intentionally not part of the authored deterministic contract. |
| `K2.FindComponentByTag` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `ComponentClassPath` | ComponentClassPath must resolve to a UActorComponent-derived class during type resolution and is normalized before planning. Under UE_5.7 this lowers to AActor::FindComponentByTag with a typed Tag input and the return pin conformed from the metadata-driven ComponentClass input. The dynamic ComponentClass pin is intentionally not part of the authored deterministic contract. |
| `K2.GetComponentsByTag` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `ComponentClassPath` | ComponentClassPath must resolve to a UActorComponent-derived class during type resolution and is normalized before planning. Under UE_5.7 this lowers to AActor::GetComponentsByTag with a typed Tag input and the array element type conformed from the metadata-driven ComponentClass input. The dynamic ComponentClass pin is intentionally not part of the authored deterministic contract. |
| `K2.SpawnActor` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `ActorClassPath` | ActorClassPath must resolve to an AActor-derived class during type resolution and is normalized before planning. Under UE_5.7 this lowers through the generic node-spawner path to UK2Node_SpawnActorFromClass with the fixed SpawnActorFromClass surface: SpawnTransform, CollisionHandlingOverride, TransformScaleMethod, Owner, ReturnValue, and class-specific ExposeOnSpawn property pins. SpawnTransform must be authored and connected because the UE_5.7 node expands through by-reference transform calls. The dynamic Class and WorldContextObject pins are intentionally not part of the authored deterministic contract. |
| `K2.Delay` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `none` | Lowers to UKismetSystemLibrary::Delay. |
| `K2.Cast` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `TargetClassPath` | TargetClassPath must resolve to a class during type resolution and is normalized before planning. Under UE_5.7 the backing UK2Node_DynamicCast is instantiated through the generic node-spawner path, preserving pure versus impure cast parity from the authored exec-pin surface. |
| `K2.Reroute` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `none` | Creates a knot node through the generic node-spawner path. |
| `K2.Select` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `IndexPinCategory`, `ValuePinCategory` | IndexPinCategory currently supports only bool, int, or enum under UE 5.7. Optional IndexObjectPath, ValueObjectPath, and NumOptions metadata refine the wildcard shape. Enum index selects use IndexObjectPath for the enum, explicit type metadata is resolved before planning, and the backing UK2Node_Select is instantiated through the generic node-spawner path. Unsupported index/value connections now fail with dedicated apply-time diagnostics. |
| `K2.SwitchInt` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `none` | Planned exec output pins define the case labels and must parse as integers. The backing UK2Node_SwitchInteger is instantiated through the generic node-spawner path, and unsupported selection-pin type combinations now fail with dedicated apply-time diagnostics. |
| `K2.SwitchString` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `none` | Planned exec output pins define the case labels. Optional CaseSensitive metadata configures comparison behavior, the backing UK2Node_SwitchString is instantiated through the generic node-spawner path, and unsupported selection-pin type combinations now fail with dedicated apply-time diagnostics. |
| `K2.SwitchEnum` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `EnumPath` | EnumPath must resolve to a UEnum during type resolution and is normalized before planning. The backing UK2Node_SwitchEnum is instantiated through the generic node-spawner path, and unsupported selection-pin type combinations now fail with dedicated apply-time diagnostics. |
| `K2.FormatText` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `FormatPattern` | Creates a format-text node through the generic node-spawner path and reconstructs argument pins from the format pattern. |
| `K2.MakeStruct` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `StructPath` | StructPath must resolve to a UScriptStruct during type resolution and is normalized before planning. |
| `K2.BreakStruct` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `StructPath` | StructPath must resolve to a UScriptStruct during type resolution and is normalized before planning. |
| `K2.MakeArray` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `ValuePinCategory` | Optional ValueObjectPath and NumInputs metadata refine the container shape. NumInputs must be at least 1, explicit value-type metadata resolves before planning, and the backing UK2Node_MakeArray is instantiated through the generic node-spawner path. |
| `K2.MakeSet` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `ValuePinCategory` | Optional ValueObjectPath and NumInputs metadata refine the container shape. NumInputs must be at least 1, explicit value-type metadata resolves before planning, and the backing UK2Node_MakeSet is instantiated through the generic node-spawner path. |
| `K2.MakeMap` | `Exact` | `any` | `EventGraph`, `UserConstructionScript` | `KeyPinCategory`, `ValuePinCategory` | Optional KeyObjectPath, ValueObjectPath, and NumPairs metadata refine the map shape. NumPairs must be at least 1, explicit key/value type metadata resolves before planning, and the backing UK2Node_MakeMap is instantiated through the generic node-spawner path. |
| `K2.BindDelegate.<PropertyName>` | `Prefix` | `any` | `EventGraph`, `UserConstructionScript` | `none` | Optional OwnerClassPath constrains delegate-property resolution. Without it, the symbol pass resolves document-authored dispatchers first, then existing Blueprint dispatchers, then inherited/native delegate properties. |
| `K2.RemoveDelegate.<PropertyName>` | `Prefix` | `any` | `EventGraph`, `UserConstructionScript` | `none` | Optional OwnerClassPath constrains delegate-property resolution. Without it, the symbol pass resolves document-authored dispatchers first, then existing Blueprint dispatchers, then inherited/native delegate properties. |
| `K2.ClearDelegate.<PropertyName>` | `Prefix` | `any` | `EventGraph`, `UserConstructionScript` | `none` | Optional OwnerClassPath constrains delegate-property resolution. Without it, the symbol pass resolves document-authored dispatchers first, then existing Blueprint dispatchers, then inherited/native delegate properties. |
| `K2.CallDelegate.<PropertyName>` | `Prefix` | `any` | `EventGraph`, `UserConstructionScript` | `none` | Optional OwnerClassPath constrains delegate-property resolution. Without it, the symbol pass resolves document-authored dispatchers first, then existing Blueprint dispatchers, then inherited/native delegate properties. |
| `K2.CreateDelegate.<FunctionName>` | `Prefix` | `any` | `EventGraph`, `UserConstructionScript` | `none` | Creates a delegate node and uses a finalize pass to assign the selected function after initial graph compilation. The selected function resolves during compilation against target-graph custom events, document-authored functions, and existing Blueprint or parent-class functions. |
<!-- END GENERATED SUPPORTED NODE CONTRACT TABLE -->
