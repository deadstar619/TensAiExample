...
- `FVergilCompileResult` now includes `Statistics` plus ordered `PassRecords`. Compiler-produced results report target graph, requested/effective schema version, requested auto-layout/comment flags, normalized per-phase command counts, plan fingerprints, planning/apply invocation counts, apply-side undo/redo transaction audits, and the last completed or failed compiler pass without requiring log scraping.
...
- `FVergilCompileResult.Statistics.TransactionAudit` is populated only for apply paths. It records whether Vergil opened a scoped editor transaction, the explicit Vergil transaction context/title/primary Blueprint object, and before/after undo-stack snapshots including queue length, undo count, next undo/redo transaction ids, titles, contexts, primary objects, and whether the target Blueprint is referenced by the undo buffer.
