...
- `UVergilAgentSubsystem::ExecuteRequest(...)` is the supported orchestration entry point for those agent requests. Planning stays read-only, apply requests replay only the explicit provided command plan, failed apply requests now auto-recover their own recorded Vergil transaction when that rollback is safe, and each execution appends an audit entry automatically.
...
- `UVergilAgentSubsystem::ExecuteRequest(...)` now normalizes missing plan target paths from `Document.BlueprintPath`, defaults missing plan graph names from `UVergilDeveloperSettings::DefaultTargetGraphName` before falling back to `EventGraph`, trims `WriteAuthorization` text fields, normalizes apply command ordering before fingerprint checks, and records the normalized request in the audit trail.
