...
| `K2.ForLoopWithBreak` | any | none | Optional `MacroBlueprintPath` and `MacroGraphName`. Defaults resolve to the engine `ForLoopWithBreak` macro in `StandardMacros`, and the symbol pass validates the selected macro graph before planning. |
...
| `K2.Gate` | any | none | Optional `MacroBlueprintPath` and `MacroGraphName`. Defaults resolve to the engine `Gate` macro in `StandardMacros`, and the symbol pass validates the selected macro graph before planning. The primary exec input follows the engine name `Enter`, and authored `StartClosed` pins normalize to the engine bool input `bStartClosed`. |
| `K2.WhileLoop` | any | none | Optional `MacroBlueprintPath` and `MacroGraphName`. Defaults resolve to the engine `WhileLoop` macro in `StandardMacros`, and the symbol pass validates the selected macro graph before planning. |
